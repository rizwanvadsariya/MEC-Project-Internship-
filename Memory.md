...
**Current stage:** Phase 0 is fully complete. Phase 1 Steps 6–13 are now implemented: role-based empty-state dashboards, a read-only province-wide scheme browser, RD team assembly, the DG approval queue with rejection/resubmission, site-visit auto-creation/display, department-specific dynamic form-template seeding, lead-MEO visit-form filling, and private photo capture/upload with signed reads. The live reject → resubmit → approve loop is covered by a real automated test (not just a one-off manual script), and it asserts exactly one site visit is created on approval. The next Phase 1 work is issue reporting (Step 14).
